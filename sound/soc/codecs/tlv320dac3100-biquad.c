/*
 * linux/sound/soc/codecs/tlv320dac3100-biquad.c
 *
 *
...
#include "tlv320dac3100-biquad.h"
#include "tlv320dac3100_eq_coeff.h" 		/* Eq Coefficient Header file */
...
extern unsigned int dac3100_read(struct snd_soc_codec *codec, unsigned int reg);
 ...
 ***************************************************************************** 
 */
int dac3100_eq_select = DAC3100_HP_EQ_SELECT;

/* dac3100_biquad_array
 * There are two Pages related to the biquad Array. Buffer A and B and hence
 * we will maintain an array of the dac3100_parse_data structure
 */
dac3100_parse_data  dac3100_biquad_array[ADAPTIVE_MAX_CONTROLS];
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
  ...
  u8 page = (kcontrol->private_value) & DAC3100_8BITS_MASK;
  u8 reg = (kcontrol->private_value >> 8) & DAC3100_8BITS_MASK;
  u8 rmask = (kcontrol->private_value >> 16) & DAC3100_8BITS_MASK;
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
  ...
  u8 page = (kcontrol->private_value) & DAC3100_8BITS_MASK;
  u8 reg = (kcontrol->private_value >> 8) & DAC3100_8BITS_MASK;
  u8 wmask = (kcontrol->private_value >> 24) & DAC3100_8BITS_MASK;
...
#define SOC_ADAPTIVE_CTL_DAC3100(xname, page, reg, read_mask, write_mask) \
...
static struct snd_kcontrol_new snd_adaptive_controls[] = {
  SOC_ADAPTIVE_CTL_DAC3100 (FILT_CTL_NAME_DAC, BUFFER_PAGE_DAC, 0x1, 0x4,
			    0x4),
  SOC_ADAPTIVE_CTL_DAC3100 (COEFF_CTL_NAME_DAC, BUFFER_PAGE_DAC, 0x1, 0x2,
			    0x1),
};
...
biquad_adaptive_filter_mixer_controls (struct snd_soc_codec *codec)
{
  ...
  for (i = 0; i < ARRAY_SIZE (snd_adaptive_controls); i++)
      ...
      if (err < 0)
	{
      ...
      dprintk("Adding %d %s \r\n", i, snd_adaptive_controls[i].name); 
    }
  return 0;
}

/***************************************************************************
 * Function: dac3100_add_biquad_controls
 *
 ...
 */
void
dac3100_add_biquads_controls (struct snd_soc_codec *codec)
{
...
/*************************DAC3100 EQ Selection *******************************/

/***************************************************************************
 * Function: dac3100_get_EQ_Select
 *
 * Purpose:  Add amixer kcontrols for DAC3100 EQ selection controls,
 *
 *       int - 0 on success and -1 on failure
 */
static int
dac3100_get_EQ_Select (struct snd_kcontrol *kcontrol,
    struct snd_ctl_elem_value *ucontrol)
{
  ucontrol->value.integer.value[0] = dac3100_eq_select;
  return 0;
}

/*
 *----------------------------------------------------------------------------
 * Function : dac3100_put_EQ_Select
 *
 * Purpose  : put routine for amixer kcontrols, write user values to registers
 *            values. Used for for DAC3100 'MUX control' amixer controls.
 *----------------------------------------------------------------------------
 */
static int
dac3100_put_EQ_Select (struct snd_kcontrol *kcontrol,
          struct snd_ctl_elem_value *ucontrol)
{
  ...
  if( index == -1) {
  	for (index = 0; index < ARRAY_SIZE (MUX_dac3100_controls); index++) {
	   if (strstr ("Stereo_Mux_1", MUX_dac3100_control_names[index]))
	    break;
 	}
	if( index == ARRAY_SIZE (MUX_dac3100_controls)) {
	  index = -1;
	  return (ret_val);
 	}
  }

  page = MUX_dac3100_controls[index].control_page;

  user_value ++;

  if (user_value < DAC3100_HP_EQ_SELECT || user_value > DAC3100_DIRECT_EQ_SELECT)
  	return (ret_val);
 
  if (index < ARRAY_SIZE (MUX_dac3100_controls))
    {
      dprintk ("Index %d Changing to Page %d\r\n", index,
        MUX_dac3100_controls[index].control_page);

      dac3100_change_page (codec, MUX_dac3100_controls[index].control_page);

      data[1] = (u8) ((user_value >> 8) & DAC3100_8BITS_MASK);
      data[2] = (u8) ((user_value) & DAC3100_8BITS_MASK);

      /* start register address */
      data[0] = MUX_dac3100_controls[index].control_base;
      ...
      if (ret_val != MUX_CTRL_REG_SIZE + 1) {
	   ...
	   dac3100_eq_select = ucontrol->value.integer.value[0];
	   ret_val = 0;
...
static const char *dac3100_EQ_Select_Str[] = {"Headphone EQ", "Speaker EQ","Direct"};

static const struct soc_enum miniDSP_EQ_Select_enum[] = {
 SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(dac3100_EQ_Select_Str), dac3100_EQ_Select_Str),
};

/***************************************************************************
 * Function: dac3100_EQ_Select_mixer_controls
 *
 ...
 */
static int
dac3100_EQ_Select_mixer_controls (struct snd_soc_codec *codec)
{
  int err;

  snd_mode_controls.name = "DAC3100 EQ Selection";
  ...
  snd_mode_controls.get = dac3100_get_EQ_Select;
  snd_mode_controls.put = dac3100_put_EQ_Select;
...
/***************************************************************************
 * Function: dac3100_add_EQ_mixer_controls
 *
 * Purpose: Configures the AMIXER Controls for the miniDSP run-time configuration.
 * 	    This routine configures the function pointers for the get put and the info
 *          members of the snd_mode_controls and calls the snd_ctl_add routine
 ...
 */
int
dac3100_add_EQ_mixer_controls (struct snd_soc_codec *codec)
{
  int err;

  err = dac3100_EQ_Select_mixer_controls (codec);
...
/***************************************************************************
 * Function: dac3100_parse_biquad_array
 *
 * Purpose: Configures the AMIXER Controls for the miniDSP run-time configuration.
 * 	    This routine configures the function pointers for the get put and the info
 *          members of the snd_mode_controls and calls the snd_ctl_add routine
 ...
 */
int
dac3100_parse_biquad_array (struct snd_soc_codec *codec)
{
  u16 index = 0;
  u16 count = 0;
  u16 jump;
  ...
  /*totalbytecount variable denotes the total number of values sent by the user 
   *and it can span across a single page.
   */
  while (index < ADAPTIVE_MAX_CONTROLS) {
      ...
      if (miniDSP_D_reg_values[count].reg_off == 0) {
        dac3100_biquad_array[index].page_num = miniDSP_D_reg_values[count].reg_val;
      ...
      dac3100_biquad_array[index].burst_array[array_index++] = regoffset = miniDSP_D_reg_values[count+1].reg_off;
      dac3100_biquad_array[index].burst_array[array_index++] = miniDSP_D_reg_values[count+1].reg_val;
      ...
      for (jump = 0; (((regoffset + array_index) < 128) && 
        ...
        if (miniDSP_D_reg_values[count].reg_off != (miniDSP_D_reg_values[count - 1].reg_off + 1))
            ...
            dac3100_biquad_array[index].burst_array[array_index++] = miniDSP_D_reg_values[count].reg_val;
        }
      }
                       
      /* Now Configure the Burst_Tranfer Length into the Array */
      dac3100_biquad_array[index].burst_size = array_index;
      printk("Page %d has %d Coefficient Values \r\n", dac3100_biquad_array[index].page_num,
        dac3100_biquad_array[index].burst_size);
  ...
  while (index < ADAPTIVE_MAX_CONTROLS) {
    ...
    dac3100_change_page (codec, dac3100_biquad_array[index].page_num);
    
    ret_val = i2c_master_send (i2c, &dac3100_biquad_array[index].burst_array[0], 
                              dac3100_biquad_array[index].burst_size);
    
    if (ret_val != dac3100_biquad_array[index].burst_size) {
	    ...
	    dprintk("Wrote %d Values into Page %d Start RegOffset %d\r\n",
			dac3100_biquad_array[index].burst_size, 
                        dac3100_biquad_array[index].page_num,
        	        dac3100_biquad_array[index].burst_array[0]);    
    }
    index++;
  }

  dac3100_change_page (codec, BUFFER_PAGE_DAC);
...
revert:
 ...
 /***************************************************************************
 * Function: dac3100_update_biquad_array
 *
 * Purpose: Configures the AMIXER Controls for the miniDSP run-time configuration.
 * 	    This routine configures the function pointers for the get put and the info
 *          members of the snd_mode_controls and calls the snd_ctl_add routine
 ...
 */
int
dac3100_update_biquad_array (struct snd_soc_codec *codec, int speaker_active, int playback_active)
{
  u16 index = 0;
  char data[2];
  struct i2c_client *i2c = codec->control_data;
  ...
  dprintk("update_biquad Status %d\r\n", playback_active);

  if (speaker_active)
    dac3100_eq_select = DAC3100_SPK_EQ_SELECT;
  else
    dac3100_eq_select = DAC3100_HP_EQ_SELECT;
  ...
  if (i2c_master_send (i2c, data, 1) != 1) {
	...
	if (i2c_master_recv (i2c, data, 1) != 1) {
	  ...
	  if (i2c_master_send (i2c, data, 2) != 2) {
	     ...
	     dprintk("Updated for Buffer Switch Old: %x New: %x\r\n", dac_ram_value, data[1]);
   	  }
       }
  } 	
  /* Check for the playback active flag. */
