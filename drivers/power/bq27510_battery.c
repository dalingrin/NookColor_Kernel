...
static int bq27510_battery_temperature(struct bq27510_device_info *di)
{
	...
	/* Battery temperature is returned in multiple of 0.1 K */
	return ((temp/KELVIN_SCALE_RANGE) - OFFSET_KELVIN_CELSIUS);
}
...
static int bq27510_battery_supply_prop_present(struct bq27510_device_info *di)
{
	...
	return ((bat_det_flag & (1 << BAT_DET_FLAG_BIT))?1:0);
}

/*
 * Return predicted remaining battery life at the present rate of discharge, in minutes.
 */
...
/*
 * Return predicted remaining time until the battery reaches full charge, in minutes
  */
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_PRESENT:
		...
		val->intval = bq27510_battery_time_to_empty_now(di);
		break;
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:		
		val->intval = bq27510_battery_time_to_full_now(di);
		break;	
	case POWER_SUPPLY_PROP_CHARGE_FULL:		
		val->intval = bq27510_battery_max_level(di);
		break;	
	default:
		return -EINVAL;
	}
